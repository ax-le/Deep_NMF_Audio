...
def multilayer_beta_NMF(data, all_ranks, beta = 1, delta = 1e-6, n_iter_max_each_nmf = 100, init_each_nmf = "nndsvd", return_errors = False, verbose = False):
    ...
    for i in range(1, L): # Layers
        W_i, H_i, error_i, toc_i = one_layer_update(data=W[i - 1], rank=all_ranks[i], beta=beta, delta=delta, init_each_nmf=init_each_nmf, n_iter_max_each_nmf=n_iter_max_each_nmf, verbose=verbose)
        W[i], H[i], reconstruction_errors[i], toc[i] = W_i, H_i, error_i, toc_i
        if verbose:
...
def one_layer_update(data, rank, beta, delta, init_each_nmf, n_iter_max_each_nmf, verbose):
    ...
    toc = np.sum(times)
    return W_normalized, H_normalized, reconstruction_error, toc
